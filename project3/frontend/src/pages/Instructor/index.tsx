...
export default function InstructorPage() {
  ...
  const createMut = useMutation({
    mutationFn: () =>
      createMyCourse({
        ...
        instructorWebhookUrl: form.instructorWebhookUrl || undefined,
      }),
    onSuccess: () => {
  ...
  const openEdit = (course: Course) => {
    ...
    setForm({
      ...
      instructorWebhookUrl: (course as any).instructorWebhookUrl ?? '',
    });
    setShowForm(true);
  };
      ...
      {showForm && (
        <div className="bg-white border border-gray-200 rounded-xl p-5 mb-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">가격 (원, 0 = 무료)</label>
              <input
                type="number"
                min="0"
                step="1000"
            ...
            <div className="col-span-2">
              <label className="block text-xs text-gray-500 mb-1">
                Webhook URL <span className="text-gray-400">(선택 · 강의 승인/반려 시 이벤트 수신)</span>
              </label>
              <input
                value={form.instructorWebhookUrl}
                onChange={(e) => setForm((f) => ({ ...f, instructorWebhookUrl: e.target.value }))}
                className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400"
